...
import service from 'services/fancythings/post'
import { successContentReponse, successSimpleResponse } from 'utils/common'
import { DELETE_FT_POST_SUCCESS } from 'constants/message'
...
const get = catchAsync(async (req: any, res: any) => {
  const data = await service.get(req.body.id)
  res.status(httpStatus.OK).send(successContentReponse(data))
})

const deletePost = catchAsync(async (req: any, res: any) => {
  await service.deletePost(req.body.id)
  res.status(httpStatus.OK).send(successSimpleResponse(DELETE_FT_POST_SUCCESS))
})
