<template>
  <div>
    <el-row type="flex" class="row-bg" justify="space-between">
      ...
      <el-col v-if="!detail.image.id" :span="6">
        <el-upload
    ...
    <el-dialog :visible.sync="dialogVisible">
      <cld-image v-if="detail.image.id" :cloud-name="detail.image.cloudName" :public-id="detail.image.publicId" />
    </el-dialog>
  </div>
</template>
<script lang="ts">
...
export default class ImageComp extends Vue {
  ...
  get requestData() {
    return {
      objectId: this.$route.params.ingredientId,
      objectType: CONTENT_IMAGE_OBJECT_TYPE.INGREDIENT,
      isCover: CONTENT_IMAGE_IS_COVER.DISABLE
    }
  }

  handleRemove() {
    const id = this.$store.state.ingredient.detail.item.image.id
    this.$store.dispatch('image/delete', id).then(() => {
      this.$store.commit('ingredient/SET_DETAIL_ITEM_IMAGE', {})
    })
  }
  ...
  onSuccess(response) {
    ...
    this.$store.commit('ingredient/SET_DETAIL_ITEM_IMAGE', image)
  }
}
</script>
<style lang="scss">
