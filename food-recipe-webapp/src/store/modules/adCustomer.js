...
import { AD_CUSTOMER } from '@/api/endpoint'
...
const mutations = {
  ...
  SET_FORM_PHONE: (state, value) => { state.form.phone = value },
  SET_FORM_EMAIL: (state, value) => { state.form.email = value },
  SET_FORM_WEBSITE: (state, value) => { state.form.website = value }
}

const actions = {
  getAllItem({ commit, state }) {
    return new Promise((resolve, reject) => {
      sendRequest(METHOD.POST, AD_CUSTOMER.GET_ALL, pickBy(state.list.query))
        .then(res => {
          const { data } = res
  ...
  getDetailItem({ commit }, id) {
    return new Promise((resolve, reject) => {
      sendRequest(METHOD.POST, AD_CUSTOMER.GET, { id })
        .then(({ data }) => {
  ...
  upsert(context, payload) {
    return new Promise((resolve, reject) => {
      sendRequest(METHOD.POST, AD_CUSTOMER.UPSERT, payload)
  ...
  delete(context, id) {
    return new Promise((resolve, reject) => {
      sendRequest(METHOD.POST, AD_CUSTOMER.DELETE, { id })
        .then(res => resolve(res))
        .catch(err => reject(err))
    })
  }
