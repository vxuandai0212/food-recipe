...
import { RECIPE_KEYWORD } from '@/api/endpoint'
...
const mutations = {
  ...
  SET_FORM_KEYWORD_NAME: (state, value) => { state.form.keywordName = value }
}

const actions = {
  getAllItem({ commit, state }) {
    return new Promise((resolve, reject) => {
      sendRequest(METHOD.POST, RECIPE_KEYWORD.GET_ALL, pickBy(state.list.query))
        .then(res => {
          const { data } = res
  ...
  upsert(context, payload) {
    return new Promise((resolve, reject) => {
      sendRequest(METHOD.POST, RECIPE_KEYWORD.INSERT, payload)
  ...
  delete(context, id) {
    return new Promise((resolve, reject) => {
      sendRequest(METHOD.POST, RECIPE_KEYWORD.DELETE, { id })
        .then(res => resolve(res))
        .catch(err => reject(err))
    })
  }
