...
import { NUTRITION } from '@/api/endpoint'
...
const mutations = {
  ...
  SET_FORM_UNIT_ID: (state, value) => { state.form.unitId = value }
}

const actions = {
  getAllItem({ commit, state }) {
    return new Promise((resolve, reject) => {
      sendRequest(METHOD.POST, NUTRITION.GET_ALL, pickBy(state.list.query))
        .then(res => {
          const { data } = res
  ...
  upsert(context, payload) {
    return new Promise((resolve, reject) => {
      sendRequest(METHOD.POST, NUTRITION.UPSERT, payload)
  ...
  delete(context, ids) {
    return new Promise((resolve, reject) => {
      sendRequest(METHOD.POST, NUTRITION.DELETE, { ids })
        .then(res => resolve(res))
        .catch(err => reject(err))
    })
  }
