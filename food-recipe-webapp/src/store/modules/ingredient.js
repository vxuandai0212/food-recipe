...
import { INGREDIENT } from '@/api/endpoint'
...
const mutations = {
  ...
  SET_FORM_DISPLAY_PRICE: (state, value) => { state.form.displayPrice = value },
  SET_FORM_DISPLAY_PRICE_UNIT: (state, value) => { state.form.displayPriceUnit = value }
}

const actions = {
  getAllItem({ commit, state }) {
    return new Promise((resolve, reject) => {
      sendRequest(METHOD.POST, INGREDIENT.GET_ALL, pickBy(state.list.query))
        .then(res => {
          const { data } = res
  ...
  getDetailItem({ commit }, id) {
    return new Promise((resolve, reject) => {
      sendRequest(METHOD.POST, INGREDIENT.GET, { id })
        .then(({ data }) => {
  ...
  upsert(context, payload) {
    return new Promise((resolve, reject) => {
      sendRequest(METHOD.POST, INGREDIENT.UPSERT, payload)
  ...
  delete(context, id) {
    return new Promise((resolve, reject) => {
      sendRequest(METHOD.POST, INGREDIENT.DELETE, { id })
        .then(res => resolve(res))
        .catch(err => reject(err))
    })
  }
