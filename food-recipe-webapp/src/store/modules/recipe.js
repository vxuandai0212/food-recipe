...
import { RECIPE } from '@/api/endpoint'
...
const mutations = {
  ...
  SET_FORM_COOK_TIME: (state, value) => { state.form.cookTime = value }
}

const actions = {
  getAllItem({ commit, state }) {
    return new Promise((resolve, reject) => {
      sendRequest(METHOD.POST, RECIPE.GET_ALL, pickBy(state.list.query))
        .then(res => {
          const { data } = res
  ...
  getDetailItem({ commit }, id) {
    return new Promise((resolve, reject) => {
      sendRequest(METHOD.POST, RECIPE.GET, { id })
        .then(({ data }) => {
  ...
  upsert(context, payload) {
    return new Promise((resolve, reject) => {
      sendRequest(METHOD.POST, RECIPE.UPSERT, payload)
  ...
  delete(context, id) {
    return new Promise((resolve, reject) => {
      sendRequest(METHOD.POST, RECIPE.DELETE, { id })
        .then(res => resolve(res))
        .catch(err => reject(err))
    })
  }
